...
public class SessionTest {
    ...
    public void testUnsupportedVersion() throws Exception {
        ...
        assertEquals(MsgType.LOGOUT, lastToAdminMessage.getHeader().getString(FixTags.MSG_TYPE));
        assertEquals(
    ...
    public void testLogonIsFirstMessageOnAcceptor() throws Exception {
        ...
        assertEquals(MsgType.LOGON,
                application.toAdminMessages.get(0).getHeader().getString(FixTags.MSG_TYPE));
    ...
    public void testStartOfInitiatorOutsideOfSessionTime() throws Exception {
        ...
        assertEquals(MsgType.LOGON, logon.getHeader().getString(FixTags.MSG_TYPE));
        assertEquals(2, state.getNextSenderMsgSeqNum());
        assertEquals(1, state.getNextTargetMsgSeqNum());
        ...
        assertEquals(MsgType.LOGOUT, logout.getHeader().getString(FixTags.MSG_TYPE));
    ...
    public void testSequenceRollbackOnCallbackException() throws Exception {
        ...
        final Application application = new UnitTestApplication() {
            ...
            public void fromAdmin(Message message, SessionID sessionId) throws FieldNotFound,
                    IncorrectDataFormat, IncorrectTagValue, RejectLogon {
                super.fromAdmin(message, sessionId);
                if (message.getHeader().getString(FixTags.MSG_TYPE).equals(MsgType.LOGON)) {
                    return;
                }
                throw new RuntimeException("TEST");
            }
    ...
    public void testNonLogonMessageNonFIXT() throws Exception {
        ...
        final SessionID sessionID = new SessionID(FixVersions.BEGINSTRING_FIX44, "SENDER", "TARGET");
        final StringField applVerID = MessageUtils.toApplVerID(FixVersions.BEGINSTRING_FIX44);
        final UnitTestApplication application = new UnitTestApplication();
        final Session session = SessionFactoryTestSupport.createSession(sessionID, application,
    ...
    public void testNonLogonMessageFIXT() throws Exception {
        ...
        final SessionID sessionID = new SessionID(FixVersions.BEGINSTRING_FIXT11, "SENDER",
                "TARGET");
        final StringField applVerID = MessageUtils.toApplVerID(FixVersions.FIX50SP2);
        final UnitTestApplication application = new UnitTestApplication();
        final Session session = SessionFactoryTestSupport.createSession(sessionID, application,
    ...
    public void testCatchErrorsFromCallbackAndSendReject() throws Exception {
        ...
        try {
            ...
            assertEquals(MsgType.NEWS,
                    application.lastFromAppMessage().getHeader().getString(FixTags.MSG_TYPE));
            assertEquals(MsgType.BUSINESS_MESSAGE_REJECT, application.lastToAppMessage()
            ...
            assertEquals(MsgType.HEARTBEAT, application.lastFromAdminMessage().getHeader()
                    .getString(FixTags.MSG_TYPE));
            assertEquals(MsgType.REJECT,
                    application.lastToAdminMessage().getHeader().getString(FixTags.MSG_TYPE));

            session.next(createAdminMessage(4));
            assertEquals(5, session.getExpectedTargetNum());
            assertEquals(5, session.getExpectedSenderNum());
            assertEquals(MsgType.TEST_REQUEST, application.lastFromAdminMessage().getHeader()
                    .getString(FixTags.MSG_TYPE));
            assertEquals(MsgType.HEARTBEAT,
                    application.lastToAdminMessage().getHeader().getString(FixTags.MSG_TYPE));
        } catch (final Throwable t) {
    ...
    public void testAcceptorRelogon() throws Exception {
        ...
        assertFalse(Logout.MSGTYPE.equals(lastToAdminMessage.getHeader().getString(FixTags.MSG_TYPE)));
        session.close();
    }
