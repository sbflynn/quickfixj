...
public class DefaultValidator implements Validator {
     ...
     *
     * @param checkFieldsOutOfOrder The checkFieldsOutOfOrder to set.
     * @since 2.0
     */
     ...
     *
     * @param checkFieldsHaveValues The checkFieldsHaveValues to set.
     * @since 2.0
     */
     ...
     *
     * @param checkUserDefinedFields The checkUserDefinedFields to set.
     * @since 2.0
     */
     ...
     *
     * @param checkUnorderedGroupFields The checkUnorderedGroupFields to set.
     * @since 2.0
     */
     ...
     *
     * @param allowUnknownMessageFields The allowUnknownMessageFields to set.
     * @since 2.0
     */
     ...
     */
    @Override
    public void validate(FIXMessageDictionary dictionary, FIXMessage message, boolean bodyOnly) {
    ...
    private void iterate(FIXMessageDictionary dictionary, FIXMessage graph) {
        ...
        for (FIXField<?> field : graph) {
            ...
            FieldDictionary fieldDictionary = dictionary.getFieldDictionary(field.getTag());
            ...
            if (shouldCheckTag(field)) {
                ...
                if (field instanceof FIXGroupField<?>) {
                    ...
                    GroupDictionary groupDictionary = dictionary.getGroupDictionary(groupField
                            .getTag());
    ...
    private void iterate(FIXMessageDictionary dictionary, FIXMessageHeader header) {

        FIXBeginString msgBeginString = FIXBeginString.parse(header
                .getFieldValue(FIXTag.BEGIN_STRING));

        if (msgBeginString != beginString) {
            throw new UnsupportedVersion("Message version '" + msgBeginString.getValue()
                    + "' does not match the data dictionary version '" + beginString.getValue()
                    + "'");
        }
            ...
            if (shouldCheckTag(field)) {
                ...
                if (!dictionary.isHeaderField(field.getTag())) {
                    throw new FieldException(SessionRejectReason.INVALID_TAG_NUMBER,
                            SessionRejectReason.getMessage(SessionRejectReason.INVALID_TAG_NUMBER),
                            field.getTag());
                }
    ...
    private void iterate(FIXMessageDictionary dictionary, FIXMessageTrailer header) {
     ...
     * @since 2.0
     */
    private void iterate(FIXMessageDictionary messageDictionary, GroupDictionary dictionary,
            FIXGroup group) {
     ...
     * @since 2.0
     */
    private void checkValidTagNumber(GraphDictionary dictionary, FIXField<?> field) {
     ...
     * @since 2.0
     */
    private void checkHasRequired(GraphDictionary dictionary, FIXFieldGraph graph) {
        ...
        for (int field : dictionary.getRequiredFields()) {
            if (!graph.isFieldSet(field)) {
                throw new FieldException(SessionRejectReason.REQUIRED_TAG_MISSING,
                        SessionRejectReason.getMessage(SessionRejectReason.REQUIRED_TAG_MISSING),
                        field);
            }
        }
    }
    ...
    private void checkValue(FieldDictionary dictionary, FIXField<?> field) throws IncorrectTagValue {
     ...
     */
    public boolean isFieldValue(FieldDictionary dictionary, FIXField<?> field, String value) {
    ...
    private void checkValidFormat(FieldDictionary fieldDictionary, FIXField<?> field) {
