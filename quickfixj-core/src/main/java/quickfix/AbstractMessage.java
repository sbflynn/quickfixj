 ...
 * @since 2.0
 */
public abstract class AbstractMessage extends AbstractFieldGraph implements
		FIXMessage {

	private static final long serialVersionUID = -3193357271891865972L;

	private static final DecimalFormat CHECKSUM_FORMAT = new DecimalFormat(
			"000");

	private static final List<Integer> HEADER_PRE_FIELDS = Collections
			.unmodifiableList(Arrays.asList(FIXTag.BEGIN_STRING,
					FIXTag.BODY_LENGTH, FIXTag.MSG_TYPE));

	private static final List<Integer> TRAILER_POST_FIELDS = Collections
			.unmodifiableList(Arrays.asList(FIXTag.CHECK_SUM));
	...
	public int bodyLength() {
		...
		return calculateLength(getHeader()) + calculateLength(this)
				+ calculateLength(getTrailer());
	}

	private String checksum() {

		return CHECKSUM_FORMAT
				.format((calculateChecksum(getHeader())
						+ calculateChecksum(this) + calculateChecksum(getTrailer())) & 0xFF);
	}
