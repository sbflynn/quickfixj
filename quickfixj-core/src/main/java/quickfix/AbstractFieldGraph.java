...
public abstract class AbstractFieldGraph implements FIXFieldGraph, Serializable {
    ...
    private static void appendField(StringBuilder buffer, FIXFieldGraph graph, int tag) {
    ...
    protected static void calculateString(FIXFieldGraph graph, StringBuilder buffer,
            List<Integer> preFields, List<Integer> postFields) {
    ...
    protected int calculateLength(FIXFieldGraph graph) {
        ...
        for (final FIXField<?> field : graph) {
            ...
            default:
                result += MessageUtils.length(CharsetSupport.getCharsetInstance(), field);
                break;
            }
    ...
    protected int calculateChecksum(FIXFieldGraph graph) {
        ...
        for (final FIXField<?> field : graph) {
            if (field.getTag() != FIXTag.CHECK_SUM) {
                result += MessageUtils.checksum(CharsetSupport.getCharsetInstance(), field);
            }
    ...
    public FIXFieldGraph clone() {
        ...
        try {
            ...
            clone.fields = new TreeMap<Integer, FIXField<?>>(clone.fieldOrder != null
                    ? new FieldOrderComparator()
                    : null);
    ...
    /**
     * This will be expensive as it requires recursion through the fields on every call. 
     * Added for completeness only - this object should never be used as a map key. 
     *
     * @author stephen.flynn@jftechnology.com
     * @since 2.0
     */
    ...
    /**
     * Two graphs are equal iff each and every field in this graph has an equal field in the other.
     *
     * @author stephen.flynn@jftechnology.com
     * @since 2.0
     */
