...
import org.quickfixj.spi.SystemDictionaryFactory;

/**
 * MessageParser - parses a string into a {@link FIXMessage}. Intended to be threadsafe iff constructed with threadsafe args.
 *
...
class MessageParser {
    ...
    public FIXMessage parse(FIXApplication application, String input, boolean doValidation) {
        ...
        messageDictionary = dataDictionary.getMessageDictionary(application, msgType);

        if (messageDictionary == null) {
            messageDictionary = new SystemDictionaryFactory("UNKNOWN",
                    dataDictionary.getBeginString()).getMessageDictionary(application, msgType);
            message = new Message("UNKNOWN");
            message.setParseException(new FieldException(SessionRejectReason.INVALID_MSGTYPE,
                    SessionRejectReason.getMessage(SessionRejectReason.INVALID_MSGTYPE)));
            parseHeader(message, tokenIterator,
                    new GenericMessageBuilder(dataDictionary.getBeginString(), messageDictionary),
        ...
        try {
            ...
            parseHeader(message, tokenIterator, builder, messageDictionary, doValidation);
            parseBody(message, tokenIterator, builder, messageDictionary, doValidation);
    ...
    public <T extends FIXMessage> T parse(FIXApplication application, String input,
            boolean doValidation, T message) {
    ...
    private void parseHeader(FIXMessage message, MessageTokenIterator tokenIterator,
            FIXMessageBuilder messageBuilder, FIXMessageDictionary messageDictionary,
            boolean doValidation) {

        if (doValidation) {
            final boolean validHeaderFieldOrder = isNextField(message.getHeader(), tokenIterator,
                    messageBuilder, messageDictionary, FIXTag.BEGIN_STRING)
                    && isNextField(message.getHeader(), tokenIterator, messageBuilder,
                            messageDictionary, FIXTag.BODY_LENGTH)
                    && isNextField(message.getHeader(), tokenIterator, messageBuilder,
                            messageDictionary, FIXTag.MSG_TYPE);
        ...
        while (!tokenIterator.finished()
            ...
            FIXField<?> field = parseNextField(message.getHeader(), tokenIterator, messageBuilder,
                    messageDictionary);

            if (field instanceof FIXGroupField<?>) {
                parseGroup(message.getHeader(), tokenIterator, messageBuilder, messageDictionary,
                        (FIXGroupField<?>) field);
            }
        }
    }

    private void parseBody(FIXMessage message, MessageTokenIterator tokenIterator,
            FIXMessageBuilder messageBuilder, FIXMessageDictionary messageDictionary,
            boolean doValidation) {
        ...
        while (!tokenIterator.finished()) {
            ...
            if (messageDictionary.isHeaderField(tokenIterator.getTag())) {
                ...
                FIXField<?> field = parseNextField(message.getHeader(), tokenIterator,
                ...
                FIXField<?> field = parseNextField(message, tokenIterator, messageBuilder,
                        messageDictionary);

                // Group case
                if (field instanceof FIXGroupField<?>) {
                    parseGroup(message, tokenIterator, messageBuilder, messageDictionary,
                            (FIXGroupField<?>) field);
                }
            }
        }
    ...
    private void parseTrailer(FIXMessage message, MessageTokenIterator tokenIterator,
            FIXMessageBuilder messageBuilder, FIXMessageDictionary messageDictionary) {
        ...
        while (!tokenIterator.finished()) {
            ...
            parseNextField(message.getTrailer(), tokenIterator, messageBuilder, messageDictionary);
        }
    }

    private FIXField<?> parseNextField(FIXFieldGraph fields, MessageTokenIterator iterator,
            GraphBuilder<?> builder, GraphDictionary dictionary) {
        ...
        if (fields.isFieldSet(field.getTag())) {
            throw new FieldException(SessionRejectReason.TAG_APPEARS_MORE_THAN_ONCE,
                    SessionRejectReason.getMessage(SessionRejectReason.TAG_APPEARS_MORE_THAN_ONCE),
                    field.getTag());
        }
    ...
    private boolean isNextField(FIXFieldGraph fields, MessageTokenIterator iterator,
            GraphBuilder<?> builder, GraphDictionary dictionary, int tag) {
    ...
    private FIXField<?> extractField(FIXFieldGraph fields, MessageTokenIterator iterator,
            GraphBuilder<?> builder, GraphDictionary dictionary) {
        ...
        if (dictionary.isGroupField(tag) && !(field instanceof FIXGroupField<?>)) {
            ...
            field = new GenericGroupField<FIXGroup>(tag, info.getDelimiterField(),
                    iterator.getBuffer(), iterator.getCurrentSeparator() + 1, iterator.length());
        }

        return field;
        //
        //        return dictionary.produceField(builder, iterator.getTag(), iterator.getBuffer(),
        //                iterator.getCurrentSeparator() + 1, iterator.length());
    }
    ...
    private <G extends FIXGroup> void parseGroup(FIXFieldGraph parent,
            MessageTokenIterator iterator, GraphBuilder<?> builder, GraphDictionary dictionary,
            FIXGroupField<G> groupField) {

        GraphBuilder<G> groupBuilder = builder.create(groupField);
        final int firstField = groupField.getDelimiterField();

        GroupDictionary groupDictionary = dictionary.getGroupDictionary(groupField.getTag());
        ...
        while (!iterator.finished()) {
            ...
            if (!groupDictionary.isField(iterator.getTag())
                    && !groupBuilder.getFieldBuilders().containsKey(iterator.getTag())) {
                return;
            }

            FIXField<?> field = extractField(group, iterator, groupBuilder, groupDictionary);
            ...
            if (tag == firstField) {
                ...
                parseGroup(group, iterator, groupBuilder, groupDictionary, (FIXGroupField<?>) field);
                ...
                if (validator.isCheckUnorderedGroupFields()) {
                    List<Integer> fieldOrder = groupDictionary.getOrderedFields();
    ...
    private void validateCheckSum(FIXMessage message, MessageTokenIterator iterator) {
        ...
        try {
            ...
            if (checksum != MessageUtils.checksum(iterator.getMessage())) {
                ...
                throw new InvalidMessage("Expected CheckSum="
                        + MessageUtils.checksum(iterator.getMessage()) + ", Received CheckSum="
                        + checksum + " in " + iterator.getMessage());
            }
        } catch (final FieldNotFound e) {
            throw new InvalidMessage("Field not found: " + e.field + " in " + iterator.getMessage());
        }
    }
