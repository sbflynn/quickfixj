...
public class ConnectorJmxExporter {
    ...
    public ObjectName register(JmxExporter jmxExporter, SessionConnector connector) {
        return register(jmxExporter, connector,
                Integer.toString(connectorIdCounter.incrementAndGet()));
    }
    ...
    public ObjectName register(JmxExporter jmxExporter, SessionConnector connector,
        ...
        try {
            ...
            if (connector instanceof AbstractSocketAcceptor) {
                connectorAdmin = new SocketAcceptorAdmin(jmxExporter,
                        (AbstractSocketAcceptor) connector, connectorName, sessionExporter);
            } else if (connector instanceof AbstractSocketInitiator) {
                connectorAdmin = new SocketInitiatorAdmin(jmxExporter,
                        (AbstractSocketInitiator) connector, connectorName, sessionExporter);
            } else {
