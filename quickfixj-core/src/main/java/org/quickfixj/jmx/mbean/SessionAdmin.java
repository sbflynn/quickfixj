...
public class SessionAdmin extends NotificationBroadcasterSupport implements SessionAdminMBean,
    ...
    @Override
    public void resetSequence(int nextSeqNum) throws SessionNotFoundException {

        logInvocation("resetSequence to: " + nextSeqNum);
        FIXMessage sequenceReset = new Message(FixMessageTypes.SEQUENCE_RESET);
        sequenceReset.getHeader().setField(
                new GenericField(FIXTag.MSG_TYPE, FixMessageTypes.SEQUENCE_RESET));
        sequenceReset.setField(new GenericField(FIXTag.NEW_SEQ_NO, nextSeqNum));
