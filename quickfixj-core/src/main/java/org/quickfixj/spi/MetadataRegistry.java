...
public class MetadataRegistry {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(MetadataRegistry.class);

    private final List<Engine> engines = new ArrayList<Engine>();
    private final Map<String, DataDictionaryConfig> registeredConfigs = new HashMap<String, DataDictionaryConfig>();
    ...
    private final Map<BeginStringKey, SystemMessageBuilderFactory> messageBuilderFactories = new HashMap<BeginStringKey, SystemMessageBuilderFactory>();
    private final Map<BeginStringKey, MetadataDictionaryFactory> dictionaryFactories = new HashMap<BeginStringKey, MetadataDictionaryFactory>();

    private boolean built = false;

    public synchronized void register(Engine engine, Map<String, DataDictionaryConfig> configs) {
        engines.add(engine);
        registeredConfigs.putAll(configs);
    }

    public synchronized void loadServices() {
        DictionaryServiceLoader.registerSystemFactories(this);
    }

    /**
     * Should be called after all metadata has been loaded from the classpath and 
     * other configuration resources but before this registry is put into service.
     * @since 2.0
     */
    public synchronized void build() {
        ...
        for (Engine engine : engines) {
            if (engine.getTransportDictionary() != null) {
                ...
                BeginStringKey key = new BeginStringKey(engine.getTransportDictionary());

                LOG.debug(String.format("Registered transportMetadata %s", key));

                TransportMetadata metadata = new TransportMetadata(engine.getTransportDictionary()
                        .getBeginString(), registeredConfigs.get(engine.getTransportDictionary()
                        .getDictionary()));
        ...
        for (Engine engine : engines) {
            if (engine.getApplicationDictionary() != null) {
                ...
                ApplicationKey key = new ApplicationKey(engine.getApplicationDictionary());

                LOG.debug(String.format("Registered applicationMetadata %s", key));

                ApplicationMetadata metadata = new ApplicationMetadata(engine
                        .getApplicationDictionary().getApplication(), registeredConfigs.get(engine
                        .getApplicationDictionary().getDictionary()));
        ...
        for (Engine engine : engines) {
            if (engine.getMessageFactory() != null) {
                if (engine.getMessageFactory().getTransport() != null) {
                    BeginStringKey key = new BeginStringKey(engine.getMessageFactory()
                            .getNamespace(), engine.getMessageFactory().getTransport());

                    LOG.debug(String.format("Created transportBuilderProvider %s", key));

                    transportBuilderProviders.put(
                            key,
                            new SessionBuilderProvider(transportMetadata.get(key), engine
                                    .getMessageFactory()));
                }
            }
        }
        ...
        for (Engine engine : engines) {
            if (engine.getMessageFactory() != null) {
                if (engine.getMessageFactory().getApplication() != null) {
                    ApplicationKey key = new ApplicationKey(engine.getMessageFactory()
                            .getNamespace(), engine.getMessageFactory().getApplication());

                    LOG.debug(String.format("Created applicationBuilderProvider %s", key));

                    applicationBuilderProviders.put(key, new ApplicationBuilderProvider(
                            applicationMetadata.get(key), engine.getMessageFactory()));
                }
            }
        }
        ...
        for (Engine engine : engines) {
            if (engine.getApplicationDictionary() != null) {
                for (Transport transport : engine.getApplicationDictionary().getTransport()) {
                    BeginStringKey key = new BeginStringKey(engine.getApplicationDictionary()
                            .getNamespace(), transport.getBeginString());

                    if (!messageBuilderFactories.containsKey(key)) {
                        messageBuilderFactories.put(key,
                                new SystemMessageBuilderFactory(engine.getApplicationDictionary()
                                        .getNamespace(), transport.getBeginString()));
                        LOG.debug(String.format("Created messageBuilderFactory %s", key));
                    }

                    if (!dictionaryFactories.containsKey(key)) {
                        BeginStringKey keyMetadata = new BeginStringKey(transport.getNamespace(),
                                transport.getBeginString());
                        TransportMetadata tMetadata = transportMetadata.get(keyMetadata);
                        if (tMetadata != null) {
                            dictionaryFactories.put(key, new MetadataDictionaryFactory(
                                    key.namespace, tMetadata));
                            LOG.debug(String.format("Created dictionaryFactory %s", keyMetadata));
                        } else {
                            LOG.warn(String
                                    .format("Unable to create dictionaryFactory %s for application %s",
                                            keyMetadata, engine.getApplicationDictionary()
                                                    .getApplication()));
                        }
                    }
                }
        ...
        for (Engine config : engines) {
            if (config.getApplicationDictionary() != null) {
                ...
                ApplicationDictionary application = config.getApplicationDictionary();

                ApplicationKey applicationKey = new ApplicationKey(application);

                ApplicationMetadata aMetadata = applicationMetadata.get(applicationKey);
                ApplicationBuilderProvider applicationBuilderProvider = applicationBuilderProviders
                        .get(applicationKey);

                LOG.info(String.format("Building application %s", applicationKey));

                for (Transport transport : application.getTransport()) {

                    BeginStringKey applicationBeginStringKey = new BeginStringKey(
                            application.getNamespace(), transport.getBeginString());
                    BeginStringKey transportBeginStringKey = new BeginStringKey(
                            transport.getNamespace(), transport.getBeginString());

                    SystemMessageBuilderFactory factory = messageBuilderFactories
                    ...
                    MetadataDictionaryFactory dictionaryFactory = dictionaryFactories
                            .get(applicationBeginStringKey);
                    ...
                    if (sessionBuilderProvider != null && applicationBuilderProvider != null) {
                        ...
                        for (FIXMessageBuilder builder : sessionBuilderProvider
                            ...
                            fieldBuilders.putAll(sessionBuilderProvider.getHeaderFieldBuilders());
                            fieldBuilders.putAll(sessionBuilderProvider.getTrailerFieldBuilders());

                            FIXMessageBuilder mb = new SystemMessageBuilder(fieldBuilders,
                                    sessionBuilderProvider.getBeginString(), builder);

                            factory.addTransportBuilder(builder.getMsgType(), mb);
                            factory.addMessageBuilder(application.getApplication(),
    ...
    public synchronized void register(FIXEngine engine) {
        ...
        for (Entry<BeginStringKey, SystemMessageBuilderFactory> entry : messageBuilderFactories
                .entrySet()) {
            engine.registerMessageBuilderFactory(entry.getKey().beginString,
                    entry.getKey().namespace, entry.getValue());
        }

        for (Entry<BeginStringKey, MetadataDictionaryFactory> entry : dictionaryFactories
                .entrySet()) {
    ...
    public synchronized FIXMessageDictionaryFactory getMessageDictionaryFactory(
        ...
        return dictionaryFactories.get(new BeginStringKey(namespace, beginString));
    }
    ...
    public synchronized FIXMessageBuilderFactory getMessageBuilderFactory(
            FIXBeginString beginString, String namespace) {
        return messageBuilderFactories.get(new BeginStringKey(namespace, beginString));
    }
    ...
    private final class BeginStringKey {
        ...
        public BeginStringKey(Engine.TransportDictionary transportDictionary) {
            this(transportDictionary.getNamespace(), transportDictionary.getBeginString());
        }
        ...
        public boolean equals(Object obj) {
            ...
            return namespace.equals(key.namespace) && beginString == key.beginString;
        }
        ...
        public String toString() {
            return String.format("BeginStringKey %s:%s", namespace, beginString);
        }
    }
    ...
    private final class ApplicationKey {
        ...
        public ApplicationKey(Engine.ApplicationDictionary applicationDictionary) {
            this(applicationDictionary.getNamespace(), applicationDictionary.getApplication());
        }
        ...
        public boolean equals(Object obj) {
            ...
            return namespace.equals(key.namespace) && application == key.application;
        }
        ...
        @Override
        public String toString() {
            return String.format("ApplicationKey %s:%s", namespace, application);
        }
    }
}
