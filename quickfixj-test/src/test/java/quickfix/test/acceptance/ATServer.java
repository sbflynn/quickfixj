...
public class ATServer implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (fixVersions.contains("fix40")) {
                acceptFixVersion(FIXBeginString.FIX40);
            }

            if (fixVersions.contains("fix41")) {
                acceptFixVersion(FIXBeginString.FIX41);
            }

            if (fixVersions.contains("fix42")) {
                acceptFixVersion(FIXBeginString.FIX42);
            }

            if (fixVersions.contains("fix43")) {
                acceptFixVersion(FIXBeginString.FIX43);
            }

            if (fixVersions.contains("fix44")) {
                acceptFixVersion(FIXBeginString.FIX44);
            }

            if (fixVersions.contains("fix50")) {
                acceptFixVersion(FIXBeginString.FIXT11);
            }
            ...
            try {
                ...
                if (e.getCause() instanceof BindException) {
                    log.warn("Acceptor port " + port
                            + " is still bound! Waiting 60 seconds and trying again...");
                    Thread.sleep(60000);
                    acceptor.start();
                }
            }
                ...
                if (!await) {
                    ...
                    if (threadIds != null) {
                        for (long threadId : threadIds) {
                            ...
                            deadlockedThreads.add(threadInfo.getThreadId() + ": "
                                    + threadInfo.getThreadName() + " state: "
    ...
    public void acceptFixVersion(FIXBeginString beginString) {
        SessionID sessionID = new SessionID(beginString, "ISLD", "TW");
        settings.setString(sessionID, "BeginString", beginString.getValue());
    ...
    public static void main(String[] args) {
        ...
        server.acceptFixVersion(FIXBeginString.FIX42);
        server.run();
    }
