...
public class SessionTest {
	...
	public void testDisposalOfFileResources() throws Exception {
		...
		final SessionID sessionID = new SessionID(FIXBeginString.FIX44,
				"SENDER", "TARGET");
		...
		final Session session = new Session(application,
				...
				engine.getMessageBuilderFactory(FIXBeginString.FIX44,
						"org.quickfixj.messages.bd"), new DefaultValidator(
						FIXBeginString.FIX44), 30, false, 30, true, true,
	...
	public void testNondisposableFileResources() throws Exception {
		...
		final Session session = new Session(application,
				mockMessageStoreFactory, sessionID,
	...
	public void testSessionWithoutValidateSequenceNumbers() throws Exception {
		...
		FIXMessage logonRequest = MessageUtils.parse(session,
				responder.sentMessageData);
		session.next(createLogonResponse(sessionID, logonRequest, 1));

		assertEquals(1, MessageUtils.coerceToInt(application
				.lastToAdminMessage().getHeader(), FIXTag.MSG_SEQ_NUM));
	...
	public void testPossDupMessageWithoutOrigSendingTime() throws Exception {
		...
		SessionID sessionID = new SessionID(FIXBeginString.FIX44, "SENDER",
				"TARGET");
	...
	public void testInferResetSeqNumAccepted() throws Exception {
		...
		final FIXMessage logonRequest = MessageUtils.parse(session,
				responder.sentMessageData);
		final FIXMessage logonResponse = createLogonResponse(sessionID,
				logonRequest, 1);
	...
	public void testUnsupportedVersion() throws Exception {
		...
		FIXMessage logonRequest = MessageUtils.parse(session,
				responder.sentMessageData);
		FIXMessage logonResponse = createLogonResponse(sessionID, logonRequest,
				1);
		session.next(logonResponse);
		...
		newsMessage.getHeader().setField(
				new GenericField(FIXTag.BEGIN_STRING, FIXBeginString.FIX40
						.getValue()));
		session.next(newsMessage);
		final FIXMessage lastToAdminMessage = application.lastToAdminMessage();
		assertEquals(FixMessageTypes.LOGOUT, lastToAdminMessage.getHeader()
				.getFieldValue(FIXTag.MSG_TYPE));
		assertEquals(
				"Incorrect BeginString",
				lastToAdminMessage
						.getFieldValue(org.quickfixj.messages.bd.fix44.field.Text.TAG));
	...
	public void testLogonIsFirstMessageOnAcceptor() throws Exception {
		...
		assertEquals(FixMessageTypes.LOGON, application.toAdminMessages.get(0)
				.getHeader().getFieldValue(FIXTag.MSG_TYPE));
	...
	public void testStartOfInitiatorOutsideOfSessionTime() throws Exception {
		...
		assertEquals(FixMessageTypes.LOGON,
				logon.getHeader().getFieldValue(FIXTag.MSG_TYPE));
		assertEquals(2, state.getNextSenderMsgSeqNum());
		assertEquals(1, state.getNextTargetMsgSeqNum());
		FIXMessage createLogonResponse = createLogonResponse(new SessionID(
				FIXBeginString.FIX44, "TARGET", "SENDER"), logon, 1);
		...
		assertEquals(FixMessageTypes.LOGOUT,
				logout.getHeader().getFieldValue(FIXTag.MSG_TYPE));
	...
	public void testLogonOutsideSessionTimeIsRejected() throws Exception {
		...
		assertEquals(1, application.toAdminMessages.size());
		assertEquals(FixMessageTypes.LOGOUT, application.toAdminMessages.get(0)
				.getHeader().getFieldValue(FIXTag.MSG_TYPE));
	...
	private void setupFileStoreForQFJ357(final SessionID sessionID,
			...
			FieldConversionException, FileNotFoundException, IOException {
	...
	public void testRejectLogonWithSessionStatus() throws Exception {
		...
		logonTo(setUpSession(application, false, new UnitTestResponder()));
		assertFalse(application.lastToAdminMessage().isFieldSet(
				FIXTag.SESSION_STATUS));
	}
	...
	public void testSendingTimeRejectBeforeLogon() throws Exception {
		...
		Session session = setUpSession(new UnitTestApplication(), false,
				new UnitTestResponder());
		...
		message.getHeader().setField(
				new BeginString(FIXBeginString.FIX44.getValue()));
	...
	public void testResendMessagesWithIncorrectChecksum() throws Exception {
		...
		final Session session = SessionFactoryTestSupport.createSession(
				sessionID, application, false, false, true, true, null);
		UnitTestResponder responder = new UnitTestResponder();
		session.setResponder(responder);
		final SessionState state = getSessionState(session);
		...
		// Added - TODO these should be acquired via a MessageBuilder
		logonToSend.getHeader().setField(
				new BeginString(FIXBeginString.FIX44.getValue()));
		...
		assertEquals(5, MessageUtils.coerceToInt(application.lastToAppMessage()
				.getHeader(), FIXTag.MSG_SEQ_NUM));
		assertFalse(state.isResendRequested());
	...
	public void testResendRequestIsProcessedAndQueued() throws Exception {
		...
		assertTrue(FixMessageTypes.SEQUENCE_RESET.equals(MessageUtils
	...
	public void testNonLogonMessageNonFIXT() throws Exception {
		...
		assertTrue(session.isUsingDataDictionary());
		assertEquals(FIXApplication.FIX44,
	...
	public void testNonLogonMessageFIXT() throws Exception {
		...
		final SessionID sessionID = new SessionID(FIXBeginString.FIXT11,
				"SENDER", "TARGET");
		final UnitTestApplication application = new UnitTestApplication();
		final Session session = SessionFactoryTestSupport.createSession(
		...
		// Added - TODO these should be acquired via a MessageBuilder
		heartbeat.getHeader().setField(
				new BeginString(FIXBeginString.FIXT11.getValue()));
		heartbeat.getHeader().setField(
				org.quickfixj.messages.fixt11.field.MsgType.HEARTBEAT);
		...
		logon.setHeartBtInt(new org.quickfixj.messages.fixt11.field.HeartBtInt(
				30));
		...
		logon.getHeader().setField(
				new BeginString(FIXBeginString.FIXT11.getValue()));
		logon.getHeader().setField(
				org.quickfixj.messages.fixt11.field.MsgType.LOGON);
		...
		assertNull(session.getTargetDefaultApplicationVersionID());
		session.next(logon);
		assertEquals(FIXApplication.FIX50SP2,
				session.getTargetDefaultApplicationVersionID());
		assertTrue(session.isLoggedOn());
	...
	public void testCatchErrorsFromCallbackAndSendReject() throws Exception {
		...
		try {
			...
			assertEquals("B", application.lastFromAppMessage().getHeader()
					.getFieldValue(FIXTag.MSG_TYPE));
			assertEquals(
					FixMessageTypes.BUSINESS_MESSAGE_REJECT,
					application.lastToAppMessage().getHeader()
							.getFieldValue(FIXTag.MSG_TYPE));
			...
			assertEquals(FixMessageTypes.HEARTBEAT,
					application.lastFromAdminMessage().getHeader()
							.getFieldValue(FIXTag.MSG_TYPE));
			assertEquals(
					FixMessageTypes.REJECT,
			...
			assertEquals(FixMessageTypes.TEST_REQUEST,
					application.lastFromAdminMessage().getHeader()
							.getFieldValue(FIXTag.MSG_TYPE));
			assertEquals(
	...
	public void testSessionRegisteredCorrectly() throws Exception {
		...
		final SessionID sessionID = new SessionID(FIXBeginString.FIX42,
	...
	public void testNonpersistedGapFill() throws Exception {
		...
		resendRequest
				.setBeginSeqNo(new org.quickfixj.messages.bd.fix44.field.BeginSeqNo(
						1));
		resendRequest
				.setEndSeqNo(new org.quickfixj.messages.bd.fix44.field.EndSeqNo(
						100));
	...
	public void testLogonWithoutTargetCompID() throws Exception {
		...
		// Added - TODO these should be acquired via a MessageBuilder
		logonRequest.getHeader().setField(
				new BeginString(FIXBeginString.FIX44.getValue()));
	...
	private void testSequenceResetGapFillWithChunkSize(int chunkSize)
		...
		FIXEngine engine = DefaultEngine.getDefaultEngine();
		FIXMessageBuilderFactory builderFactory = engine
				.getMessageBuilderFactory(FIXBeginString.FIX44,
						"org.quickfixj.messages.bd");
		FIXMessageDictionaryFactory dataDictionary = engine
		...
		Session session = new Session(new UnitTestApplication(),
				new MemoryStoreFactory(), sessionID, dataDictionary, null,
				new ScreenLogFactory(true, true, true), builderFactory,
				new DefaultValidator(FIXBeginString.FIX44), isInitiator ? 30
						: 0, false, 30, true, resetOnLogon, false, false,
				false, false, false, true, false, 1.5, null,
				validateSequenceNumbers, new int[] { 5 }, false, false, false,
				true, false, true, false, null, true, chunkSize, false, false);
	...
	public void testMsgSeqNumTooHighWithDisconnectOnError() throws Exception {
		...
		Session session = new Session(new UnitTestApplication(),
				...
				new DefaultValidator(FIXBeginString.FIX44), isInitiator ? 30
						: 0, false, 30, true, resetOnLogon, false, false,
				false, false, false, true, false, 1.5, null,
				validateSequenceNumbers, new int[] { 5 }, false,
				disconnectOnError, false, true, false, true, false, null, true,
				0, false, false);
	...
	private Session setUpFileStoreSession(Application application,
			...
			IllegalAccessException, ConfigError, FieldConversionException {
	...
	private FIXMessage createLogonResponse(final SessionID sessionID,
			final FIXMessage logonRequest, int responseSequenceNumber)
			throws FieldNotFound {

		FIXEngine engine = DefaultEngine.getDefaultEngine();
		FIXMessageBuilderFactory builderFactory = engine
				.getMessageBuilderFactory(sessionID.getBeginString(),
						"org.quickfixj.messages.bd");
		FIXMessage logonResponse = builderFactory.getMessageBuilder(
				FixMessageTypes.LOGON).create();
		...
		MessageUtils.set(header, FIXTag.BEGIN_STRING, sessionID
				.getBeginString().getValue());
		MessageUtils.set(header, FIXTag.MSG_TYPE, FixMessageTypes.LOGON);
		MessageUtils.set(header, FIXTag.SENDER_COMP_ID,
				sessionID.getSenderCompID());
		MessageUtils.set(header, FIXTag.TARGET_COMP_ID,
	...
	private void logonTo(Session session, int sequence) throws FieldNotFound,
		...
		// Added - TODO these should be acquired via a MessageBuilder
		receivedLogon.getHeader().setField(
				new BeginString(FIXBeginString.FIX44.getValue()));
	...
	private void logoutFrom(Session session, int sequence)
		...
		// Added - TODO these should be acquired via a MessageBuilder
		receivedLogout.getHeader().setField(
				new BeginString(FIXBeginString.FIX44.getValue()));
	...
	private void setUpHeader(FIXSessionID sessionID, FIXMessage message,
			boolean reversed, int sequence) {

		message.getHeader().setField(
				new GenericField(FIXTag.TARGET_COMP_ID, reversed ? sessionID
						.getSenderCompID() : sessionID.getTargetCompID()));
		message.getHeader().setField(
				new GenericField(FIXTag.SENDER_COMP_ID, reversed ? sessionID
						.getTargetCompID() : sessionID.getSenderCompID()));
		message.getHeader().setField(new SendingTime(SystemTime.getDate()));
		message.getHeader().setField(
				new GenericField(FIXTag.MSG_SEQ_NUM, sequence));
	}
