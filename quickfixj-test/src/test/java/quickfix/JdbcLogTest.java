...
public class JdbcLogTest extends TestCase {
    ...
    private void doLogTest(DataSource dataSource) throws ClassNotFoundException, SQLException,
            ConfigError {
    ...
    private void dropTable(String tableName) throws SQLException {
        connection.prepareStatement("DROP TABLE " + tableName + " IF EXISTS;").execute();
    }

    private void setUpJdbcLog(boolean filterHeartbeats, DataSource dataSource)
            throws ClassNotFoundException, SQLException, ConfigError {
        ...
        sessionID = new SessionID(FIXBeginString.FIX42, "SENDER-" + now, "TARGET-" + now);
    ...
    private void assertLogData(Connection connection, int rowOffset, SessionID sessionID,
        ...
        assertNotNull(sessionID.getBeginString().getValue(), rs.getDate("time"));
        assertEquals(sessionID.getBeginString().getValue(), rs.getString("beginstring"));
    ...
    protected void initializeTableDefinitions(Connection connection) throws ConfigError {
        try {
            JdbcTestSupport.loadSQL(connection, "config/sql/hsqldb/messages_log_table.sql",
                    new JdbcTestSupport.HypersonicPreprocessor(null));
            JdbcTestSupport.loadSQL(connection, "config/sql/hsqldb/event_log_table.sql",
                    new JdbcTestSupport.HypersonicPreprocessor(null));
        } catch (Exception e) {
