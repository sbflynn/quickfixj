...
>
	...
	<xsl:template match="fix:fix">
		...
		<xsl:value-of select="$NEW_LINE" />
		<xsl:text>public class DictionaryService extends org.quickfixj.spi.DictionaryServiceSupport {</xsl:text>

		<xsl:value-of select="$NEW_LINE" />
		<xsl:value-of select="$NEW_LINE_INDENT" />
		<xsl:text>public DictionaryService() throws Exception {</xsl:text>
		<xsl:value-of select="$NEW_LINE_INDENT2" />
		<xsl:text>super("/META-INF/quickfix/</xsl:text>
